...
for gen in [100, 400]:
    for p in [0.1, 0.9]:
        mts = pickle.load(open(f'{path}/{simid}/mt_allmuts_{bn}_0.1_{simid}_{gen}_0.4_{p}_rs.pkl', 'rb'))
        mts = rs_cvt(mts)
        ...
        for cutoff in [0, 0.01]:
            ...
            for i in range(mt_freq.shape[0]):
                ...
                seqs += '\n'
            with open(f'{path}/{simid}/mt_allmuts_{bn}_0.1_{simid}_{gen}_0.4_{p}_rs_{cutoff}.phy', 'w') as f:
                f.write(seqs)
            fn = f'/mt_allmuts_{bn}_0.1_{simid}_{gen}_0.4_{p}_rs_{cutoff}.phy'
